[![](data:image/png;base64,)](https://graphql.org/)

# Basic GraphQl Course

Hello Everyone,

Thanks for visiting my GithHub Page.

In this Repo I had implemented the Basic GraphQl with some logic like **Mutation, Query, Data-fetching, communication with backend, Authentication, etc**

## Plugins

I used maximum latest version Plugin and configure with React, Mongo and Node.js

- _React-18_
- _Mongose and MongoDbSotre_
- _Express (and it dependancies like express-session, body-parser)_
- _Passport and Bcrypt (Autherization)_
- _react-router-dom (Latest)_
- _graphql and express-graphql (For Backend Support)_
- _apollo/client (For frontend support)_

## Installation and Prerequisite

 requires [Node.js](https://nodejs.org/) v14+ to run.
 requires [Mongodb](https://www.mongodb.com/) Either to be configure in local or live.

Please create ``` env ``` file and link it to the application for working properly.

**Configuration need**

```sh
PORT= ${Your port number}
MONGO_URL= ${mongodbURL}
SECRET_KEY= ${sessionKey}
```

```sh
npm i / yarn i
yarn dev
```

Please check the  ```${package.json}``` for more details on library and configuration.

## Enjoy :)
